...
boolean compatibleConditions( NC *cond1, NC *cond2 )
{
      ...
      if(cond1 == (NC*)NULL || cond2 == (NC*)NULL)
      {
		...
		//printf("LEFT FROM HERE\n");
	    return TRUE;
      }
      
    /*  if (cond1->link != NULL && cond2->link != NULL){
	      int store[1000];
	    	int k;
	  	for (k = 0; k < 1000; k++){
			store[k] = -1;
		...
		printf("\n*************************************************************************************************************\n");
		fetch_expression(cond1, store, 0);
		printf("\n");
		printf("cond1 = %d", cond1->link->link->inc);
		
	  	for (k = 0; k < 1000; k++){
			store[k] = -1;
		}
		printf("\n*************************************************************************************************************\n");
		fetch_expression(cond2, store, 0);
		printf("\n");
		printf("cond2 = %d\n", cond2->link->link->inc);
	}
	else{
...
int cnt = 0;
  for( i=0; i < P0->numpaths; i++ )
  {
    ...
    printf("stateeeee = %s total paths = %d\n", M0->states[P0->paths[i].start].state_id, i);
    //exit(0);
...
printf("REached here111\n");

	
	PATHS_LIST P0, P1, P00, P11;
    ...
    printf("hello1\n");
    for(i = 0; i < P0.numpaths; i++){
   		//printf("path start = %s, path end = %s\n", M1->states[P0.paths[i].start].state_id, M1->states[P0.paths[i].end].state_id);
      		//find_Ralpha(&pl->paths[i]);
    	}
	//Associate_R_and_r_alpha(M0, &P0, &V0, 0); // computes R_alpha & r_alpha
...
printf("hello3\n");
    ...
    findpaths(M1,&P1, &V1);
   // Associate_R_and_r_alpha(M1, &P1, &V1, 1);
    //printVariable(&V0);
    //printVariable(&V1);
...
/*
  ...
  /* for (i = 0; i < P0.numpaths; i++){
	...
	fetch_expression(P0.paths[i].condition, store, 0);
	printf("\n");
	if (P0.paths[i].condition != NULL && P0.paths[i].condition->link != NULL){
		printf("-- = %d", P0.paths[i].condition->link->link->inc);
	}
    }*/
    //exit(0);
   // printf("\n\n\n\n\n#############################################################################################################\n");
   /*  for (i = 0; i < P1.numpaths; i++){
	...
	//printf("j = %d, num of paths = %d", j, gammaLoop[0][idx].numberOfPaths);
	printf("\n*************************************************************************************************************\n");
	fetch_expression(P1.paths[i].condition, store, 0);
    }*/
	...
	XYZ(M0, &P0, &V0, M1, &P1, &V1, matching);
