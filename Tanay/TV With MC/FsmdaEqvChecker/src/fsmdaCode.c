 ...
 */
 ...
 generate_smv_code( FSMD *M0, int k_state_num, int init_state, int *var_kripke, int no_of_kripke_vars )
 {
  ...
  char state_id[1000], transitions[10000], next_state_id[1000], temp_state_id[1000], temp_transitions[1000], temp_next_state_id[200], sym_value[200];
   ...
   */
	printf("INIT_STATE = %d hey = %s\n", init_state, M0->states[init_state].state_id);
...
int highVariableIndex1[20];
int highVariableIndex2[20];
void //DISPLAY_A_PATH <- for finding fast
FindExplicitLeak(PATH_ST *p, int idx, int ix, var_list* V) {
	printf("\nentry\n");
	int i, j, k;
//	***************************************initializing gamma ****************************************************************************************
	/*for (i = 0; i < 20; i++){
...
//	***************************************Reading high variable ****************************************************************************************
	...
	if (ix == 0){
		...
		for (i = 0; i < 20; i++){
			highVariableIndex[i] = highVariableIndex1[i];
		}
	}
	else{
		for (i = 0; i < 20; i++){
			highVariableIndex[i] = highVariableIndex2[i];
		}
	}
...
//	***************************************Reading salpha and ralpha****************************************************************************************	
	...
	while(temp1!=NULL) {
		...
		int lhsIndex = -1;
		for (i = 0; i < V->no_of_elements; i++){
			if (V->var_val[i] == temp1->Assign.lhs){
				lhsIndex = i;
				break;
			}
		}
...
void graphTraversal(FSMD* M1, PATHS_LIST *pl, int final, int end, var_list* V1, int explicitleakage[2][mx][mx], int implicitleakage[2][mx][mx], int netDependency[2][mx][mx], int netN[2][mx][mx], FSMD* M2, PATHS_LIST *pl1, var_list* V2, int * matching, int insecure){
	...
	for (i = 0; i < pl->numpaths; i++){
		if (pl->paths[i].start != end){
			//printf("okay111 = %s\n", M1->states[pl->paths[i].start].state_id);
			continue;
		}
		//printf("OKAY\n");
		...
		if (M1->states[pl->paths[i].start].isLoop == 1){
			...
			printf("TOTAL PATHS in loop %d = %d\n", idx, gammaLoop[0][idx].numberOfPaths);
			...
			for (k = 0; k < sz; k++){
				...
				if (takeFromCurr){
					...
					int leaky = 0;
					for (j = 0; j < sz; j++){
						// Here it is checking transitive leakage, p1: x=h, p2: a=x;
						if (currDependency[0][k][j] == 1){
			...
			if (z != 0){
				//exit(0);
				printf("Culprit branch Detected for high input = %d\n", index);
			...
			int flag;
			if (mat[pl1->paths[matching[i]].end][highVariableIndex1[0]] == 1){
				flag = 1;
			}
			
		//	for (j = 0; j < 20 && outward[j] != -1; j++){
				
			
			for (j = 0; j < 20 && outward[j] != -1 && goFurther == 1 && flag == 1; j++){
			...
			if (flag == 0){
				for (j = 0; j < sz; j++){
					for (k = 0; k < sz; k++){
						if (explicitleakage[0][j][k] == 1){
							leakage[j][k] = 1;
						}
						if (implicitleakage[0][j][k] == 1){
							leakage[j][k] = 1;
						}
					}
				}
				...
				for (j = 0; j < sz1; j++){
					for (k = 0; k < sz1; k++){
						if (explicitleakage[1][j][k] == 1){
							leakage1[j][k] = 1;
						}
						if (implicitleakage[1][j][k] == 1){
							leakage1[j][k] = 1;
						}
					}
				}
			...
			for (k = 0; k < sz1; k++){
				...
				for (j = 0; j < sz1; j++){
					if (currDependency[1][k][j] == 1){
						takeFromCurr = 1;
						// checking self
						if (currDependency[1][k][k] == 1){
							int l = 0;
							for (l = 0; l < sz1; l++){
								if (explicitleakage[1][k][l] == 1){
									avoid = 1;
									break;
								}
							}
			...
			for (j = 0; j < sz; j++){
				...
				for (k = 0; k < sz; k++){
					if (explicitleakage[1][j][k] == 1 || implicitleakage[1][j][k] == 1){
						if (mat[pl1->paths[matching[i]].end][j] == 1){
							flag = 1;
							break;
						}
					}
			...
			if (mat[pl1->paths[matching[i]].end][highVariableIndex2[0]] == 1){
				flag = 1;
			}
...
void
XYZ(FSMD* M1, PATHS_LIST *pl1, var_list* V1, FSMD* M2, PATHS_LIST *pl2, var_list* V2, int *matching)
{

// reading high inputs from file
	FILE *f1;
	f1 = fopen( "High.txt", "r" );  /* open the file for reading the high variables  */
	if( f1 == NULL )
	{
	...
	for (i = 0; i < 20; i++){
		highVariableIndex1[i] = -1;
		highVariableIndex2[i] = -1;
	}
	printf("here\n");
	int k1 = 0, k = 0;
	while( !feof(f1) )
	{
		...
		for (i = 0; i < V2->no_of_elements; i++){
			printf("%s\n", V2->var_string[i]);
			printf("heeeeeee1\n");
			if (strcmp(var, V2->var_string[i]) == 0){
				printf("here1\n");
				highVariableIndex2[k1++] = i;
				break;
			}
		}
	...
	// k_state_num = generate_kripke_st(M2);
	// k_state_num = 11;
	// int var_kripke[100];
	// int no_of_kripke_vars = 0;
	// generate_smv_code( M2, k_state_num, 0, var_kripke, no_of_kripke_vars );
	// exit(0);
	...
	for (i = 0; i < M2->numstates; i++){
		if (M2->states[i].isTempCutPoint){
			int j = 0;
			...
			for (j = 0; j < V2->no_of_elements; j++){
				...
				var_kripke[0] = V2->var_val[j];
				
				
				FILE *fp_smv;
			  	fp_smv = fopen("kripke.smv", "r+");
				...
				while (chr != EOF)
				{
				 ...
				 int k;
				for( k = 0; k < no_of_kripke_vars; k++ )
				{
					printf("i= %d kri = %d\n", k, no_of_kripke_vars);
					fprintf( fp_smv,"SPEC\n");
					symbol_for_index( var_kripke[k], sym_value );
			        	fprintf(fp_smv, "    E [ ! D_%s U U_%s ]\n\n", sym_value, sym_value );
				}
				fclose(fp_smv);
			  	 
			  	 
			  	 // Putting initialization state
			  	 fp_smv = fopen("kripke.smv", "r+");
			  	 count_lines = 0;
				 chr = getc(fp_smv);
				 while (chr != EOF)
				 {
					...
					if (count_lines == line2){
						break;
					}
					chr = getc(fp_smv);
				    }
			  	 
			  	  fprintf( fp_smv, "  init( state ) := %s;\n", M2->states[i].state_id );
			  	 fclose(fp_smv);
				...
				if ((pid = fork()) == 0)
				 {
				     ...
				     close(fd);
				     execl("./NuSMV", "./NuSMV", "kripke.smv", NULL);
				     exit(0);
				 }
				 wait( pid );
				FILE *fp1 = fopen( "Nusmv.kripke.output", "r" );
				while( !feof( fp1 ) ) 
				{
				    fscanf( fp1, "%s", fp_string );

				    if( strcmp( fp_string, "true" ) == 0 )
				    {
				       flag = 1;
				       no_of_kripke_vars = 0;
				...
				mat[i][j] = flag;
 					// = 1 if there is any future path from ith state (that state must be cut point) where for jth variable E [! D_var[j] U U_var[j] ] property holds true 



				printf("var = %s, state = %s, flag = %d\n", V2->var_string[j], M2->states[i].state_id, flag);    
	...
	clock_t ttt;
   	...
   	for( i = 0; i < stab.numsymbols; i++ ){
   		printf("symbol index = %d repre = %s\n", stab.val_tab[i],stab.sym_tab[i]); 
	...
	for(i = 0; i < pl1->numpaths; i++){
   	...
   	// int qq;  //vis1 array is for phi in TV algo, call the bisimulation (graph traversal function)
   	queue[0] = 0;
   	tt = 1;
   	for (ptr = 0; ptr < tt; ptr++){
		...
		printf("GOING INSIDE\n");
		graphTraversal(M1, pl1, lastState, queue[ptr], V1, implicitleakage, explicitleakage, netDependency, netN, M2, pl2, V2, matching, 0);
	}
	
	
    	printf("***********************************************************FInal Leakage of the program*****************************************************************************\n");
