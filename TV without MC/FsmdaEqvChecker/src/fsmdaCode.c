 ...
 */
int generate_kripke_st(FSMD *M0)
{
	// exit(0);
	// printf("HELLO\n");
	FILE *fp_k;
...
void generate_smv_code(FSMD *M0, int k_state_num, int init_state, int *var_kripke, int no_of_kripke_vars)
{
	// exit(0);
...
int read_fsmd(char arg[], FSMD *M, var_list *V)
{
	FILE *f1;
...
NC *parseclause(char *clause, int operator, int not, NC *root)
{
...
int getfunname(char *s, char *name)
{
	int i = 0, count = 0;
	while (s[i] != '\0' && s[i] != '(')
	{
...
void getarguments(char *s, char *args[10])
{
	int i = 0, count = 0, j;
	char name[20];
	while (s[i] != '\0' && s[i] != '(')
...
NC *create_condexpr(char *op1, char *op2, int relop, int logicalop, int not, NC *root)
{
	int i = 0, numarg = 0;
	char *args[10];
	char fun[20];
	...
	if (op2[0] == '\0')
	{ // single variable or funcation with boolean status
		f = isfunction(op1);
		if (f)
		{
			if (not )
...
void parseassignments(char *s, DATA_TRANS *actions, var_list *V)
{
...
void parseA(char *A, DATA_TRANS *D, var_list *V)
{
...
/**********************************************************************************************/
void write_lists(NC *root) // WRITE_LISTS <- for finding fast
{
	char *sym_value;
...
NC *create_expr(char *op2, char op, char *op3)
{
	...
	{ // handling constants in which both operands are constants
		int val;
		switch (op)
		{
		case '+':
	...
	{ // handling single constant expressions
		S = (NC *)malloc(sizeof(NC));
		S->list = NULL;
		S->type = 'S';
		switch (op)
	...
	{ // handling expressions involving both variables
		S = (NC *)malloc(sizeof(NC));
		S->type = 'S';
		S->list = NULL;
		S->inc = 0;
...
int isconstant(char *s)
{
	...
	if (s[0] == '\0')
		return 0;
	if (s[0] == '-' || s[0] == '+')
	{ // check for the sign of the constant
		i = 1;
		while (s[i] != '\0')
		{
			if (s[i] >= '0' && s[i] <= '9')
...
int create_state(char *stateid, FSMD *M, int current_state)
{
	int i;
	for (i = 0; i < current_state; i++)
		if (!strcmp(stateid, M->states[i].state_id))
			return i;
...
void stack_push(STACK *S, int state, NC *cond, DATA_TRANS *action)
{

	if (S != NULL)
	{
		if (S->top >= STACKSIZE)
		{ // checking for stack over flow
			printf("\nStack over flow \n ");
			exit(0);
		}
		else
...
void dividepaths(FSMD *fsmd, int state, PATHS_LIST *paths, STACK *S)
{
...
int lastState = -1;
int flag = 0, lastt = -1;
void findpaths(FSMD *fsmd, PATHS_LIST *paths, var_list *V)
{
	printf("****************************************************************************finding path******************************************************************\n");
	// printf("hello hello hello\n");
	printf("%s\n", fsmd->name);
...
void printpath(FSMD *fsmd, STACK *S)
{
	int i;
	for (i = 0; i <= S->top; i++)
	{
...
void storepath(FSMD *fsmd, PATHS_LIST *paths, STACK *S)
{
	// printf("good = %d\n", S->top);
	PATH_NODE *t, *end;
	int i;
...
void displayallpaths(PATHS_LIST *pl)
{
...
void displayallpaths_withcp(PATHS_LIST *pl, FSMD *fsmd)
{
	...
	for (i = 0; i < pl->numpaths; i++)
	{
		...
		t = pl->paths[i].nodeslist;
		while (t->next != NULL)
		{ /* this prints each path in the list    */
			printf("%d %s --> ", t->state, pl->fsmd->states[t->state].state_id);
...
int vvv = 0;
void fetch_expression(NC *root, int *store, int idx) // WRITE_LISTS <- for finding fast
{
	char *sym_value;
	sym_value = (char *)malloc(1000 * sizeof(char));
...
void graphTraversal(FSMD *M1, PATHS_LIST *pl, int final, int end, var_list *V1, int explicitleakage[2][mx][mx], int implicitleakage[2][mx][mx], int netDependency[2][mx][mx], int netN[2][mx][mx], FSMD *M2, PATHS_LIST *pl1, var_list *V2, int *matching, int insecure)
{
	...
	if (final == end)
	{
		...
		{
			PATH_PAIR *tempList = initE();
			PATH_PAIR *tempE_u = initE();
			PATH_PAIR *tempE_c = initE();
			PATH_PAIR *tempe = tempE_u;
			PATH_PAIR *tempc = tempE_c;
			for (int i = 0; i < sz; ++i)
			{
				if (S[i] != 0 || RS[i] != 0 || CS[i] != 0)
				{
					tempe->p0 = i;
					tempe->p1 = (S[i] | RS[i] | CS[i]);
					tempe->next = initE();
					tempe = tempe->next;
				}
			}
...
int is_in_CPQ(CPQ *cpl, int state)
{
	CPQ *temp;
	temp = cpl;
	while (temp != NULL)
...
int r_alpha_equal(r_alpha *r1, r_alpha *r2, int *var_kripke, int *length, int *var_spec, int *no_of_spec_vars)
{
...
int checkCondition(PATH_ST *path, PATH_ST *tempPath)
{
	NC *t, *temp;
	...
	if (t == NULL)
	{ // no condition of execution of the path beta
	...
	{ // beta have some condition of execution
		if (tempPath->condition == NULL)
		{
		...
		{ // outward path has condition of execution
			canProceed = 0;
...
int addPath(PATH_ST *path, PATH_ST *path1, PATHS_LIST tempP1, FSMD *M0_kripke)
{
	FILE *fp1;
...
int findcp(FSMD *fsmd, PATH_ST *path, int k, PATHS_LIST tempP1, FSMD *M0, FSMD *M0_kripke)
{
	FILE *fp1;
...
void extendedPaths(PATHS_LIST *p0, PATH_ST *temppath, FSMD *M0, PATH_Q *F, PATHS_LIST *tF)
{
...
void equivalenceChecking_m0_to_m1(FSMD M0, FSMD M1, PATHS_LIST p0, PATHS_LIST p1, CPS_Q P, PATH_Q F, CPQ *eta, FSMD M0_kripke)
{
	CPS tempcs;
	CPQ *eta1;
	eta1 = eta;
...
void enQP(CPS_Q *P, int s0, int s1)
{
...
void unionEta(CPQ *eta, int s0, int s1)
{
	...
	if (!found)
	{ // found=0 indicates <s0, s1> is not in eta
...
NC *simplify_literals(NC *expr, NC *result)
{
 ...
 */
int search_primary_in_primary(NC *primary, NC *p, int flag)
{
	if (flag == 1)
		return 1;
	if (primary == NULL)
 ...
 *  it returns '1' else returns '-1'.
 */
int search_primary_in_term(NC *term, NC *p, int flag)
{
	if (flag == 1)
		return 1;
	if (term == NULL)
 ...
 *  it returns '1' else returns '-1'.
 */
int search_primary_in_sum(NC *sum, NC *p, int flag)
{
	if (flag == 1)
		return 1;
	if (sum == NULL)
...
NC *Substitute_in_MOD_DIV(NC *M_D_sum, r_alpha *transformations, NC *result)
{
